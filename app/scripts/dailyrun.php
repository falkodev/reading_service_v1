...
//definition du dossier parent (donc / dans la config actuelle) 
//$utils_path = dirname(__FILE__); 
//2 niveaux supérieurs (pour aller chercher le fichier utils.php pour la connexion à la BD) 
$path = dirname(dirname(dirname(__FILE__))); 
$utils_path = dirname(dirname(__FILE__)); 
//echo "utils_path=$utils_path<br>"; 
...
$jourActuel = date("N"); //echo "jourActuel = $jourActuel \r\n"; 
...
//echo "query = $query \r\n"; 
$result = mysqli_query($mysqli, $query); //echo "mysql query ok"; //echo "<br>result:"; //print_r($result); 
$destinataires = "<br>\r\n"; 
foreach($result as $index) { 
	$envoi_lecture = 0; 
	$envoi_texte = 0; 
	$tab_lang = include($utils_path.'/lang/'.$index['user_reading_param'].'.php'); 
	...
	//print_r($index); //echo "<br><br>user id = ".$index['user_first_name']." /// user first cycle day = ".$index["user_cycle_first_day"];
	//echo "<br>"; 
	...
	if($index['user_day_'.$jourActuel] == 1){ 
		...
		for($i=1;$i<8;$i++) { 
			if($index["user_day_$i"] == 1) { 
				$nbJours++; 
				if($i < $index["user_cycle_first_day"]) $beforeFirstCycleDay = 1; //if(($i <= $jourActuel ) || ($i > $jourActuel && $i >= $index["user_cycle_first_day"] && $jourActuel < $index["user_cycle_first_day"])) $numeroPortion++; 
				//2 cas de figure : 
		...
		if($semaineProchaine == 0) {$date=date("W"); /*echo " /// passage en 0";*/} 
		else {$date=date("W") + 1;/*echo " /// passage en 1";*/} 
		$fichier = $date."dbr".$nbJours.$numeroPortion.".html"; 
		//echo " /// fichier = $fichier"; 
		$file = file_get_contents($path."/www/portions_".$lang."/".$fichier); 
		...
		/*foreach($verses as $verse) { 
			//découpage par verset pour éviter d'avoir des lignes avec 998 caractères ou plus car sinon tronquées dans l'email if(strlen($verse) > 997) 
			$verse = str_replace("<input","\r\n<input",$verse); 
			//chaque verset finissant par "<input type='hidden'>", on ajoute à cet endroit un retour chariot 
			$verse = str_replace("<b>","\r\n<b>",$verse); 
			//chaque recherche de verset commençant par "<b>", on met des sauts à la ligne 
			$message_body .= $verse; 
		} */
...
$message = "Bonjour,<br><br> Le script dailyrun.php s'est bien déroulé. Un email a été envoyé à : $destinataires<br> L'équipe JW Reading"; 
