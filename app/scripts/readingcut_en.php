...
$langPortionTo = "to";
$langChapter = "Chapter";
$langTitle = "Daily portion";
$langAccountLink = "Direct link to your account";
...
// $langTitle = "Portion du jour";
// $langAccountLink = "Connexion compte personnel";
...
<img src="http://www.jwreading.com/assets/images/book.png" style="width: 32px; height: 32px;"/>&nbsp;&nbsp;&nbsp;&nbsp;JW Reading - '.$langTitle.'</a></p>
...
<a href="http://www.jwreading.com/login" style="color: #FFFFFF">'.$langAccountLink.'</a>
...

{
    ...
    if($link != null)  //si une réponse est retournée, c'est qu'il existe un lien sur le numéro de verset, renvoyant à des recherches de publications
    {
        ...
        $xpath_recherche = recupJson($url_index, "1"); //url menant au(x) lien(s) de recherche d'un verset
...
$utils_path = dirname(dirname(__FILE__)); //2 niveaux supérieurs (pour aller chercher le fichier utils.php pour la connexion à la BD)
...
//echo "jourSemaine = $jourSemaine\r\n";
//$result = mysqli_query($mysqli, 'delete from portions where portions_next_week = 0'); //chaque nouvelle semaine, les portions de la semaine passée sont supprimées
//$result = mysqli_query($mysqli, 'update portions set portions_next_week = 0 where portions_next_week = 1');//chaque nouvelle semaine, les portions de ce qui était la semaine à venir deviennent la semaine en cours
...
$url = "http://wol.jw.org/wol/dt/r1/lp-e/".$date; //url du json du texte du jour (contenant le lien vers la lecture de la semaine aussi)
echo "url = $url\r\n";
$xpath = recupJson($url, "1");
//lecture hebdomadaire normale balise : dernière class='se' trouvée
$query = $xpath->query("//p[@class='se']/a"); 
$liste_lecture = $query->item(1)->textContent;
$link = $query->item(1)->getAttribute('href');
echo "liste_lecture: $liste_lecture /// link:$link\r\n";
...
$textes = $xpath->query("//div[@id]"); //parcours des versets, qui sont contenus dans des div ayant un id (plusieurs div dans un verset parfois)
foreach($textes as $texte)//pour chaque paragraphe de versets (contenus dans un div)
{
    $contenu = $texte->textContent; //on extrait le contenu texte
    $contenu = mb_convert_encoding($contenu, 'HTML-ENTITIES', "UTF-8");
    //on remplace les "+" et "*" qui sont des liens vers des notes en bas de page ou vers d'autres versets    
    ...
    $contenu = str_replace("+","",$contenu);
    $contenu = str_replace("*","",$contenu);
    //echo "<br>texte = $contenu<br>";

    preg_match_all('#\d+.#',$contenu,$matches); //expression régulière pour chercher les entiers dans la chaine $contenu afin d'identifier les versets présents dans le paragraphe

    $compteur = 0;
    while($compteur < count($matches[0]))
    {
        $posDebut = strpos($contenu,$matches[0][$compteur]);//position de l'entier représentant le début du verset
        //$match = count($matches[0]);
        //echo "<br>posDebut = $posDebut<br>";
        //print_r($matches[0]);
        if($compteur == (count($matches[0]) - 1)) $verset = substr($contenu, $posDebut);//extraction du verset
        else
        {
            $posFin = strpos($contenu,$matches[0][$compteur + 1]);//position de l'entier représentant le début du verset suivant
        //if($posFin !== false)
        //{                    
            $longueur = $posFin - $posDebut;
            $verset = substr($contenu, $posDebut, $longueur); //extraction du verset
            //echo "$verset<br>";
        }                
        //echo "test ";
        $versetSpan = $xpath->query('.//span',$texte)->item($compteur);//parcours des spans présents dans le paragraphe (et donc des versets)
        //print_r($versetSpan);
        //echo "<br>";
        //echo "vS = $versetSpan<br>";
		//echo "json = $json<br>";
        $class = $versetSpan->getAttribute('class');//identification de sa classe pour savoir s'il possède un lien sur le numéro de verset (et donc une recherche à effectuer)
        $span = $versetSpan->textContent;
        $id = $versetSpan->getAttribute('id');

        if(substr($class,0,2) == "dc")
        {
            //echo " new chapter ";
            $numeroChapitre = trim($span);
            $numeroVerset = 1;
        }
        else if(substr($class,0,2) == "dv") $numeroVerset++;
        ...
        if($link != null)  //si une réponse est retournée, c'est qu'il existe un lien sur le numéro de verset, renvoyant à des recherches de publications
        {
            ...
            //echo "<br>json = $json /// lien de recherche href = $url_index<br>";
            //recherches : class='sx'                    
            $query = $xpath_recherche->query("//p[@class='sx']");
            //print_r($query);
            foreach ( $query as $child ) 
            {
                ...
                foreach($liens_recherche as $lien_recherche)
                {
                    ...
                    {                           
                        $xpath_recherche = recupJson($url_recherche); //url de la recherche                                
                        //echo "<br>json = $json /// lien final = $url_recherche<br>";
                        if($json == 1) //si le xpath retourné est bien issu d'un json
                            $affich = $xpath_recherche->query("//p[@class='sb']")->item(0)->textContent; 
                        else // sinon c'est un xpath issu d'un html
                        {
                            $nbElem = $parFin - $parDebut; //nb de paragraphes de la sélection(recherche en rouge sur le site web)
                            $parcours = array(); //tableau qui recueillera les id de paragraphe à garder
                            ...
                            $pieces = $xpath_recherche->query("//div[@id]"); //parcours de tous les div ayant un id
                            $affich = "";
                            foreach($pieces as $piece)
                            {
                                $id = $piece->getAttribute('id');
                                if(in_array($id, $parcours)) $affich .= $piece->textContent; //on garde seulement ceux correspondant ayant une valeur contenue dans le tableau $parcours (qui est la sélection des paragraphes de la recherche en cours)     
                            }
                            //echo "<br><b>reference = $reference</b> /// affich = $affich<br>";
                        }
                        //echo "<b>$reference :</b> $affich<br>";
                        if($affich != "" | $affich != null) 
                        {
        ...
        //if($numeroVerset == 1) $verset = "Chapitre ".$verset;
        $verset .= "<input type='hidden' value='".$numeroChapitre.":".$numeroVerset."'>"; //ajout d'un champ hidden qui servira au découpage en portions plus loin, pour savoir les versets inclus dans les portions découpées
    
        if($compteur == 0) 
        { 
            if($numeroVerset == 1)  { $versets .= "<br><br>\r\nChapter ".$verset; $verset = "<br><br>\r\nChapter ".$verset;  }
            else { $versets .= "<br><br>\r\n".$verset; $verset = "<br><br>\r\n".$verset;  }
        }
        else $versets .= $verset;
        //echo "$verset";
        ...
        $lecture[$increment][1] = $ancre;
        $increment++;
        $compteur++; //compteur de versets dans le paragraphe actuel
        $compteurVerset++; //compteur de versets pour un jour de lecture donné      
    }//fin while($compteur < count($matches[0]))
}//fin foreach($textes as $texte)
$versets .= "<br><br>\r\n\r\n/////////// Publications researches ///////////<br><br>\r\n$ancres<br><br>";
$versets = "This week's reading is $liste_lecture.<br><br>\r\n".$versets;

//structure : portions_next_week->$semaineSuivante ///	portions_type->1 à 7 /// portions_day->ex : si portions_type=3 les valeurs possibles seront 1, 2 ou 3 /// portions_content -> contenu portion
$versets = mb_convert_encoding($versets, 'HTML-ENTITIES', "UTF-8");
$semaine = date("W") + 1;
$nomFichier = $path."/www/portions_en/".$semaine."dbr11.html";
//echo "<br>nom fichier : $nomFichier<br>";
file_put_contents($nomFichier, utf8_encode($versets));
...
for($i=2;$i<8;$i++) //boucle de 2 à 7 pour envoyer les portions nécessaires pour les différents découpages possibles
{
    ...
    for($k=0;$k<$countVerses;$k++)
    {
        ...
        {
            $versets .= "<br><br>\r\n\r\n/////////// Publications researches ///////////<br><br>\r\n\r\n$ancres\r\n<br><br>";
            $versets = "This week's reading is $liste_lecture.<br><br>\r\n\r\nToday's portion goes from $premierVerset to $dernierVerset.<br><br>\r\n\r\n".$versets;
            $versets = mb_convert_encoding($versets, 'HTML-ENTITIES', "UTF-8");
            $nomFichier = $path."/www/portions_en/".$semaine."dbr".$i.$compteurPortions.".html";
            file_put_contents($nomFichier, $versets);
            $compteurPortions++;
            $versets = "";
            $ancres = "";
...
$temps_fin = microtime(true);
echo "<br>duree execution: ".round($temps_fin - $temps_deb)." secondes<br>";
//echo "<br>versets = $versets ";

    /********************************************************************************************
    *                                                                                           *
    *   supprimer fichiers html de plus de 1 semaine (test date ou sur 2 premiers caracteres    *
    *   du nom de fichier (=semaine de creation + 1))                                           *
    *                                                                                           *
    *   si json == 0 à la ligne 53, on arrête tout                                              *
    ********************************************************************************************/
...
function recupJson() 
{
    ...
    //epuration du /en/ si présent
    $return = strstr($lien, '/en');
    if($return !== false)
    {
        $expl = explode ('http://wol.jw.org/en', $lien);
        //echo "$expl[0] /// $expl[1] /// ";
        $lien = 'http://wol.jw.org'.$expl[1];
        //echo "<br>passage pour supprimer le /en de lien /// lien = $lien ";
    }
    ...
    if ($portion === false) //si échec de chargement json, on va retenter mais en html classique
    {
        ...
        $expl = explode ('http://wol.jw.org/', $lien);
        //echo "$expl[0] /// $expl[1] /// ";
        $lien = 'http://wol.jw.org/en/'.$expl[1]; //on ajoute le /en/ pour passer à la version html
        //echo "<br>changement lien ";
        $headers = get_headers($lien,true); //on regarde les en-têtes pour voir la redirection vers l'url "html"
        $location = $headers['location']; //on regarde où mène la redirection
        $expl = explode ('#h=', $location); //dans la page redirigée figure cette valeur "#h="; ce qui suit révèle l'id des paragraphes (en rouge sur le site web de jw) de la recherche (pour ne pas rapatrier tout un article quand seuls qq paragraphes sont concernés)
        if(isset($expl[1])) //s'il y a un bien la valeur "#h="
