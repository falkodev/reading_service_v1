...
foreach($vs as $keySpan => $versetSpan)
{
	...
	if($text == $versetNext){		
		...
		//if(substr($class,3,2) == "dx" and $numeroChapitre == 11 and $numeroVerset == 29) //si lien "dx", verset avec lien recherches
...
$versets = str_replace("*","",$versets);
$semaine = date("W") + 1;
$nomFichier = "portions_en_test/".$semaine."dbr11.html";
...
for($i=2;$i<8;$i++) //boucle de 2 à 7 pour envoyer les portions nécessaires pour les différents découpages possibles
{
    ...
    for($k=0;$k<$countVerses;$k++)
    {
        ...
        if($k == ($comp - 1)) 
        {
			...
			$versets = str_replace("+","",$versets);
			$versets = str_replace("*","",$versets);
            $nomFichier = "portions_en_test/".$semaine."dbr".$i.$compteurPortions.".html";
...
{
	global $versetSpan, $xpath, $numeroVerset, $numeroChapitre, $versets, $verset, $ancres, $ancre, $json;
			...
			foreach($liens_recherche as $lien_recherche)
			{
				...
				{                           
					...
					if($json == 1){ //si le xpath retourné est bien issu d'un json
						//echo "<br>longueur tableau : ".$xpath_verset->query("//p[@class='sb']")->length." ";
						$affich = "";
